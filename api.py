...
# 初始化FastAPI应用
app = FastAPI(title="InfiniteTalk API", description="API for InfiniteTalk: Audio-driven Video Generation",
              version="1.0.0")
...
async def startup_event():
    ...
    quant_dir = os.getenv("QUANT_DIR", "./weights/InfiniteTalk/quant_models/infinitetalk_single_fp8.safetensors")
    ...
    if not os.path.exists(quant_dir):
        logger.warning(f"Quant directory {quant_dir} does not exist")
    ...
    try:
        ...
        logger.info("InfiniteTalk pipeline initialized successfully with fp8 quantization")
...
async def generate_video(
        ...
        color_correction_strength: float = Form(1.0)
):
    try:
        # 创建临时工作目录
